...
public sealed class Event : Entity
{
    ...
    {
        if (Status != EventStatus.Draft) return Result.Failure(EventErrors.NotDraft);
    ...
    {
        if (StartsAtUtc == startsAtUtc && EndsAtUtc == endsAtUtc) return;
    ...
    {
        if (Status == EventStatus.Canceled) return Result.Failure(EventErrors.AlreadyCanceled);

        if (StartsAtUtc < utcNow) return Result.Failure(EventErrors.AlreadyStarted);
